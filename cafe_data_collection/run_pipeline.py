...
def get_user_confirmation(
    message: str = "Continue to the next step?", skip_confirmations: bool = False
) -> bool:
...
async def run_city_pipeline(
    pipeline: CafePipeline, city_info: dict, skip_confirmations: bool = False
) -> list[dict]:
    ...
    print(
        f"Review the output at: {pipeline.pipeline_dir}/step2_cafes_{city.replace(' ', '_').lower()}.json"
    )
    ...
    print(
        f"Review the output at: {pipeline.pipeline_dir}/step3_geocoded_{city.replace(' ', '_').lower()}.json"
    )
    ...
    print(
        f"Review the output at: {pipeline.pipeline_dir}/step4_enriched_{city.replace(' ', '_').lower()}.json"
    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the cafe data collection pipeline with steps for review"
    )

    parser.add_argument("input_csv", help="Path to CSV file containing city data")
    parser.add_argument(
        "city_mapping_json", help="Path to JSON file containing city ID mappings"
    )
    ...
    parser.add_argument("--contentful-output", help="Path for Contentful output file")
    ...
    if args.city:
        collection_queue = [
            city for city in collection_queue if city["city"] == args.city
        ]
    ...
    if step <= 4:
        for city_info in collection_queue:
            ...
            enriched_cafes = await run_city_pipeline(
                pipeline, city_info, skip_confirmations
            )
            all_cafes.extend(enriched_cafes)

            if city_info != collection_queue[-1]:
                if not get_user_confirmation(
                    "Continue to next city?", skip_confirmations
                ):
